...
One file, `.github/mc-bump.yml`, in the mod's repository. It is the only thing
...
testing the wrong thing**, which is the exact failure this repository exists to
prevent. So:
...
- an embedded newline in any string is an error — see [single line](#why-every-string-must-be-a-single-line).
...
**Required.** `fabric` is the only implementation today. See
...
`mod.id` is what the server test greps for — but not naïvely. Fabric Loader
prints the mods it loaded as an indented tree, and the test anchors on that
dash-space prefix rather than on the bare id, which also appears in every
classpath dump and stack trace.
...
in the job summary. That is the point — an unrun test rots silently. It just does
not veto, and it does not open a failure issue on its own; crying wolf for a test
the mod itself declared non-blocking would train everyone to ignore the issues.
...
The `{mc}` half names the **set** of covered versions, and the wildcard never
climbs a level: `26.1.x` yes, `26.x` never. `26.x` would promise 26.3 to anyone
reading the file name, and nothing has ever built, booted or published it.

A `mod_version` that does not match `format` is an **error**, not a warning. The
previous behaviour — warn, then leave the file alone — produced a jar whose name
...
exists. If your mod genuinely has no unit tests, set `workflows.unit-tests:
false` — that is an honest "we do not run any", where an empty green is a lie.
...
The `auto-update` matrix stays **sequential regardless**: the escalation ladder
replays the whole matrix after each bump, which is sequential by construction.
...
| `expect-count` | `[]` | Numbers the mod reports, checked against the source. |
...
| Key | Default | Meaning |
|---|---|---|
| `assignee` | `""` | GitHub login that gets the failure issue and the update PR. Empty = nobody. |
...
own red checks; opening an issue as well is noise. Turn it on for a repository
where pull requests come from forks and the author never looks at the Actions
tab.
