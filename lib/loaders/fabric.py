...
from ..toolchain import Requirements, load_table, plan_toolchain
...
# Minecraft version -> the Loom line and Gradle Fabric's example mod builds it
# with. Generated by scripts/refresh-toolchain.py, see lib/toolchain.py.
TOOLCHAIN_TABLE = Path(__file__).with_name("fabric_toolchain.json")
...
class FabricLoader(Loader):
    ...
    def resolve(
        ...
        # Loom is the build plugin, not a Minecraft dependency, and it only moves
        # when the target Minecraft version needs a newer line than the mod has:
        # see lib/toolchain.py. pin_buildtool is the escape hatch.
        plan = self._toolchain(minecraft_version, env, pin_buildtool)
        return Resolved(
            loader=loader,
            api=api,
            buildtool=plan.buildtool,
            gradle=plan.gradle,
            extra={"buildtool_note": plan.buildtool_note, "gradle_note": plan.gradle_note},
        )
    ...
    def resolve_one(
        ...
        if role == "buildtool":
            return self._toolchain(minecraft_version, env, None).buildtool
        raise Failure(f"unknown role '{role}' for the fabric loader")

    def _toolchain(self, minecraft_version: str, env: BuildEnv, pinned: str | None):
        return plan_toolchain(
            minecraft=minecraft_version,
            table=load_table(TOOLCHAIN_TABLE),
            current_buildtool=env.buildtool,
            current_gradle=env.gradle,
            java=env.java,
            candidates=self._stable_looms,
            requirements=self._loom_requirements,
            label="fabric-loom",
            pinned=pinned,
        )
    ...
    def _loom_requirements(self, version: str) -> Requirements | None:
    ...
    def _stable_looms(self) -> list[str]:
        ...
        (1.17.14 comes after 1.18.0-alpha.4), so the caller sorts it numerically
