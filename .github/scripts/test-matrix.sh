...
# claims, not just the newest one. Usable locally:
#
#   bash .github/scripts/test-matrix.sh
#
# Why a matrix. An update bumps Fabric API and the loader to the newest release
# and widens depends.minecraft over the whole series (">=26.1 <=26.1.2"). Testing
# only 26.1.2 proves nothing about 26.1 and 26.1.1 running with THAT Fabric API,
# yet those are versions the jar promises to load on and that the stores list.
# Every claimed version is therefore built and booted with the resolved
# dependencies, which is exactly the combination that ships.
#
# The list comes from scripts/update-mc-version.py --list-test-versions, so the
# notion of "series" lives in one place only.
#
...
if [ -z "${MC_VERSIONS:-}" ]; then
    MC_VERSIONS="$(python3 scripts/update-mc-version.py --list-test-versions)" || {
...
for MC in "${VERSIONS[@]}"; do
    ...
    if ! boot; then
        # Loom resolves mods on virtual threads, and they can deadlock against
        # each other on the JVM-wide Cleaner monitor while setting Minecraft up.
        # That hang never reaches the point where Minecraft itself starts, which
        # is exactly how it is told apart from the mod being broken: a mod that
        # fails to load DOES get that far. Only the hang is worth retrying.
        ...
        echo "==> Minecraft never started on $MC (loom setup hang?), retrying once"
        if ! boot; then
